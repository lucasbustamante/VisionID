...
internal object XchengAidlPrinter {
    ...
    private const val BITMAP_CALLBACK_GRACE_MS = 3_500L
        ...
        return try {
            ...
            if (!bound) {
                return PrinterAttempt(
                    ...
                    success = false,
                    backend = "Positivo/XCheng AIDL",
                    detail = "bindService retornou false ao imprimir foto; $startResult."
                )
            }
            ...
            if (!connected.await(CONNECTION_TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
                return PrinterAttempt(
                    ...
                    false,
                    "Positivo/XCheng AIDL",
                    "Tempo esgotado ao conectar para imprimir foto; $startResult."
                )
            }
            ...
            runCatching { service.printBitmap(bitmap, callback) }
                .getOrElse {
                    throw IllegalStateException(
                        "Falha ao enviar bitmap: ${it.javaClass.simpleName}: ${it.message.orEmpty()}",
                        it
                    )
                }
            ...
            runCatching { service.printWrapPaper(4, callback) }
                .getOrElse {
                    throw IllegalStateException(
                        "Falha ao avançar papel após a foto: ${it.javaClass.simpleName}: ${it.message.orEmpty()}",
                        it
                    )
                }

            val deadline = SystemClock.elapsedRealtime() + BITMAP_CALLBACK_GRACE_MS
            while (SystemClock.elapsedRealtime() < deadline) {
                callbackState.throwIfError()
                if (callbackState.completions.get() > 0) break
                SystemClock.sleep(80)
            ...
            PrinterAttempt(
                ...
                false,
                "Positivo/XCheng AIDL",
                "${error.javaClass.simpleName}: ${error.message.orEmpty()}; $startResult"
            )
        } finally {
            if (bound) runCatching { appContext.unbindService(connection) }
        }
